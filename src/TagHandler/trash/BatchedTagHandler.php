...
abstract class BatchedTagHandler extends TagHandler {
	 ...
	 * @return string[]
	 */
	public function get(string $batch): array { return $this->search("", $batch); }

	/**
	 * @param $search
	 * @return string[]
	 */
	public function search(string $search, string $batch): array {
		$filter = Filter::where(Comparison::field("batch")->is($batch));
		if ($search !== "") $filter->and(Comparison::field("tag")->instring($search));
